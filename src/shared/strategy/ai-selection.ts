...
export const DEFAULT_SELECTION_QUERY =
  '沪深 A 股，非 ST，上市超过一年，成交额大于 2 亿，收盘价高于 20 日均线，按近 20 日涨幅从高到低排序';
export const aiSelectionSettingsSchema = z
  .object({
  ...
  .strict();
export const aiSelectionQuerySchema = z
  .object({
    platform: selectionPlatformSchema,
  ...
  .strict();
export const aiSelectionKeySchema = z
  .object({
    platform: selectionPlatformSchema,
    apiKey: z
      .string()
      ...
      .min(1, '请输入 API Key')
      .max(4096)
      .regex(/^[\x21-\x7e]+$/u, 'API Key 不能包含空白或中文'),
  })
  .strict();
