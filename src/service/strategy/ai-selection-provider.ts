...
const scalarText = (value: unknown): string =>
  typeof value === 'string' ? value : typeof value === 'number' ? String(value) : '请检查接口权限和额度';
...
function stockFromRow(row: JsonObject): AiSelectionStock | null {
  ...
  const metrics = entries
    .filter(
      ([key, value]) =>
        !codeField.test(cleanLabel(key)) &&
        !nameField.test(cleanLabel(key)) &&
        !secretField.test(key) &&
        ['string', 'number', 'boolean'].includes(typeof value),
    )
    .slice(0, 24)
    .map(([label, value]) => ({ label: label.slice(0, 120), value: String(value).slice(0, 300) }));
...
export function parseSelectionPayload(
  payload: unknown,
  limit: number,
): { stocks: AiSelectionStock[]; total: number | null; explanation: string; warnings: string[] } {
  ...
  const visit = (value: unknown, depth = 0): void => {
    ...
    if (Array.isArray(value)) {
      value.slice(0, 3000).forEach((item) => visit(item, depth + 1));
      return;
    }
    if (typeof value === 'string') {
      ...
      let parsed = false;
      try {
        decoded = JSON.parse(text.replace(/^```(?:json)?\s*/u, '').replace(/\s*```$/u, ''));
        parsed = true;
      } catch {
        /* Plain text is a valid MCP response. */
      }
      if (parsed) {
      ...
      for (let i = 0; i < lines.length - 1; i++) {
        ...
        if (!headers.some((header) => codeField.test(cleanLabel(header))) || !/^\s*\|?[\s:|-]+\|?\s*$/u.test(lines[i + 1]!))
          continue;
        recognized = true;
        i += 2;
    ...
    if (!item) return;
    const failedStatus = ['status_code', 'errorcode'].some((field) => item[field] !== undefined && Number(item[field]) !== 0);
    const failedCode = typeof item.code === 'number' && ('data' in item || 'message' in item) && ![0, 200].includes(item.code);
    if (failedStatus || failedCode || item.success === false || item.isError === true || item.error) {
      throw new Error('平台返回查询失败，请检查账号权限、额度和选股条件后重试');
    }
    const hasNestedData = ['data', 'datas', 'rows', 'result', 'content'].some(
      (field) => Array.isArray(item[field]) || object(item[field]) !== null,
    );
    if (!hasNestedData && Object.keys(item).some((key) => codeField.test(cleanLabel(key)))) {
      recognized = true;
      rows.push(item);
      return;
    }
    ...
    for (const field of [
      ...
      if (Array.isArray(item[field]) && item[field].length === 0 && field !== 'content') recognized = true;
  ...
  const stocks = [
    ...new Map(
      rows
        .map(stockFromRow)
        .filter((row): row is AiSelectionStock => row !== null)
        .map((row) => [row.symbol, row]),
    ).values(),
  ];
  if (!recognized && total !== 0)
    throw new Error('平台未返回可识别的股票表格，请在条件中明确要求“返回沪深 A 股股票代码、股票简称及筛选指标”后重试');
  ...
  if ((total ?? stocks.length) > limit || stocks.length > limit)
    warnings.push(`按平台返回顺序保留前 ${limit} 只有效沪深股票；不是全市场完整结果。`);
...
export class AiSelectionProvider {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        response.status === 401 || response.status === 403
          ? '平台认证失败，请检查 API Key 与接口权限'
          : response.status === 429
            ? '平台调用额度不足或请求过于频繁，请稍后重试'
            : `平台暂不可用（HTTP ${response.status}）`,
      );
    }
    ...
    try {
      while (true) {
        ...
        if (
          ...
          sseResult(text, object(body)?.id) !== undefined
        )
          break;
      }
      text += decoder.decode();
  ...
  async query(input: AiSelectionQuery, apiKey: string): Promise<ReturnType<typeof parseSelectionPayload>> {
    ...
    try {
      ...
      // The original cause can contain secret-bearing HTTP headers; discard it deliberately.
      // eslint-disable-next-line preserve-caught-error
      throw new Error(message.split(apiKey).join('[已隐藏]').slice(0, 800));
    }
  }

  private async wencai(
    input: AiSelectionQuery,
    apiKey: string,
    signal: AbortSignal,
  ): Promise<ReturnType<typeof parseSelectionPayload>> {
    const { text } = await this.post(
      ...
      { query: input.query, page: '1', limit: String(input.limit), is_cache: '1', expand_index: 'true' },
      signal,
    );
    let result: JsonObject | null;
    try {
      result = object(JSON.parse(text.split(apiKey).join('[已隐藏]')));
    } catch {
      throw new Error('问财返回格式异常，请稍后重试');
    }
    if (!result) throw new Error('问财未返回有效数据');
    if ((result.status_code !== undefined && Number(result.status_code) !== 0) || result.error)
      throw new Error(`问财查询失败：${scalarText(result.status_msg ?? result.error)}`);
    return parseSelectionPayload(result, input.limit);
  }

  private async eastmoney(
    input: AiSelectionQuery,
    apiKey: string,
    signal: AbortSignal,
  ): Promise<ReturnType<typeof parseSelectionPayload>> {
    ...
    const rpc = async (method: string, params: unknown, notification = false): Promise<JsonObject> => {
      ...
      const { response, text } = await this.post(
        ...
        headers,
        { jsonrpc: '2.0', ...(notification ? {} : { id: requestId }), method, params },
        signal,
      );
      ...
      try {
        payload = object(
          response.headers.get('content-type')?.includes('text/event-stream')
            ? sseResult(redacted, requestId)
            : JSON.parse(redacted),
        );
      } catch {
        throw new Error('妙想返回格式异常，请稍后重试');
      }
      if (!payload) throw new Error('妙想未返回有效数据');
      if (payload.error) throw new Error(`妙想查询失败：${scalarText(object(payload.error)?.message ?? payload.error)}`);
    ...
    const initialized = await rpc('initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'trading-diary', version: '1.0.0' },
    });
...
function sseResult(text: string, id: unknown): unknown {
  for (const event of text.replace(/\r\n/gu, '\n').split('\n\n').slice(0, -1)) {
    ...
    try {
      ...
      if (object(value)?.id === id && id !== undefined) return value;
    } catch {
      /* Heartbeats and other events are ignored. */
    }
  }
  return undefined;
}
...
export function selectScreeningTool(tools: JsonObject[]): JsonObject {
  // Verified on the official endpoint: all MX tools carry the same default
  // destructiveHint=true, including this query-only securities screener.
  // Match this exact tool before generic discovery; news tool descriptions
  // also mention it as a referral and must never become selection candidates.
  const official = tools.filter(tool => tool.name === 'mx_stocks_screener');
  if (official.length === 1) return official[0]!;
  const selected = tools.filter(
    (tool) =>
      typeof tool.name === 'string' &&
      object(tool.annotations)?.destructiveHint !== true &&
      /xuangu|stock[_-]?(?:screen|select)|(?:screen|select)[_-]?stock|智能选股|条件选股/iu.test(
        `${tool.name} ${typeof tool.description === 'string' ? tool.description : ''}`,
      ) &&
      !/模拟交易|下单|买入委托|sell_order|buy_order|place_order/iu.test(
        `${tool.name} ${typeof tool.description === 'string' ? tool.description : ''}`,
      ),
  );
  if (selected.length !== 1) throw new Error('客户端未识别到唯一的妙想选股工具，请更新接口适配；这不是积分不足的返回结果');
  return selected[0]!;
}
...
export function screeningArguments(tool: JsonObject, input: AiSelectionQuery): JsonObject {
  ...
  for (const [key, value] of Object.entries(properties)) {
    ...
    if (
      /^(query|question|keyword|prompt|search_query|searchstring|query_text|queryText)$/u.test(key) &&
      property.type === 'string'
    ) {
      args[key] = input.query;
      hasQuery = true;
    } else if (/^(limit|page_size|pageSize|size|top_n)$/u.test(key) && ['number', 'integer'].includes(String(property.type)))
      args[key] = input.limit;
    else if ('default' in property) args[key] = property.default;
  }
  if (!hasQuery || (Array.isArray(schema?.required) && schema.required.some((key) => typeof key !== 'string' || !(key in args))))
    throw new Error('妙想选股参数定义已变化，暂无法安全构造查询，请等待接口适配更新');
  return args;
}
