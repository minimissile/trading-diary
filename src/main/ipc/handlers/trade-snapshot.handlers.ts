...
export function registerTradeSnapshotHandlers({ window: parent, service }: IpcHandlerContext): void {
  let job: {
    child: BrowserWindow;
    payload: Promise<TradeSnapshotPayload>;
    finish: (error?: Error, image?: string) => void;
    capturing: boolean;
  } | null = null;
  ...
  ipcMain.handle(ipcChannels.tradeSnapshotOpen, (event, raw: unknown) => {
    ...
    // Isolated sessions need their own handler for packaged renderer assets.
    child.webContents.session.protocol.handle('app', (request) => net.fetch(request.url));
    ...
    const payload = (async (): Promise<TradeSnapshotPayload> => {
      ...
      const date = new Intl.DateTimeFormat('en-CA', {
        timeZone: trade.venue === 'US' ? 'America/New_York' : 'Asia/Shanghai',
        year: 'numeric',
        month: '2-digit',
        day: '2-digit',
      });
      ...
      const [result, entries] = await Promise.all([
        service.request('market.listKlines', {
          symbol: `${trade.venue}:${trade.symbol}`,
          period: '1d',
          adjust: 'none',
          limit: 240,
          beforeTimestamp: tradeTime + 86_400_000,
        }),
      ...
      const history = entries.filter(
        (entry) => entry.venue === trade.venue && entry.id !== trade.editingId && Date.parse(entry.tradeAt) <= tradeTime,
      );
      const markers = buildChartTradeMarkers(
        [
          ...
          {
            ...trade,
            id: 'snapshot-current-trade',
            source: 'manual',
            planId: null,
            note: '本次交易（快照生成时尚未保存）',
            sipOccurrenceId: null,
            cashOutflow: null,
            createdAt: trade.tradeAt,
          },
        ],
        '1d',
      );
      ...
      if (current) {
        current.timestamp = bars[bars.length - 1]!.timestamp;
        current.label = trade.side === 'buy' ? '本买' : '本卖';
        current.color = '#e3bb73';
      }
  ...
  ipcMain.handle(ipcChannels.tradeSnapshotReady, async (event, error?: string) => {
    ...
    if (error) {
      current.finish(new Error(String(error).slice(0, 300)));
      return;
    }
