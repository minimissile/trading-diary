...
export function registerPortfolioHandlers({ window, service }: IpcHandlerContext): void {
  ...
  ipcMain.handle(ipcChannels.portfolioGetDividendCalendar, (event, input: { accountId?: string; month: string }) => {
    assertTrustedSender(event, window);
  ...
  ipcMain.handle(ipcChannels.portfolioListLedgerEntries, (event, input: { accountId?: string; symbol?: string }) => {
    assertTrustedSender(event, window);
    return service.request('portfolio.listLedgerEntries', input);
  });

  ipcMain.handle(ipcChannels.portfolioGetRealizedHistory, (event, input: { accountId?: string; year?: number }) => {
    assertTrustedSender(event, window);
    return service.request('portfolio.getRealizedHistory', input);
  });

  ipcMain.handle(ipcChannels.portfolioGetPnlCalendar, (event, input: { accountId?: string; month?: string }) => {
    assertTrustedSender(event, window);
  ...
  ipcMain.handle(ipcChannels.portfolioSyncPnlCalendarBar, (event, input: { accountId?: string; symbol: string }) => {
    assertTrustedSender(event, window);
    return service.request('portfolio.syncPnlCalendarBar', input);
  });

  ipcMain.handle(ipcChannels.portfolioUpdateLedgerEntry, (event, input: { id: string; input: Record<string, unknown> }) => {
    assertTrustedSender(event, window);
    return service.request('portfolio.updateLedgerEntry', input);
  });
  ...
  ipcMain.handle(ipcChannels.portfolioDeletePosition, (event, input: { accountId?: string; symbol: string }) => {
    assertTrustedSender(event, window);
  ...
  ipcMain.handle(ipcChannels.portfolioRefreshDividends, (event, input: { accountId?: string; symbol?: string }) => {
    assertTrustedSender(event, window);
  ...
  ipcMain.handle(
    ...
    (event, input: { accountId?: string; settings: DividendGoalSettings | null }) => {
      assertTrustedSender(event, window);
      return service.request('portfolio.saveDividendGoal', input);
    },
  );

  ipcMain.handle(ipcChannels.portfolioGetDividendPayoutDefault, (event, input: { accountId: string; symbol: string }) => {
    assertTrustedSender(event, window);
  ...
  ipcMain.handle(ipcChannels.portfolioReadLedgerImportImagePreviews, (event, input: { sourcePaths: string[] }) => {
    assertTrustedSender(event, window);
    return service.request('portfolio.readLedgerImportImagePreviews', input);
  });

  ipcMain.handle(ipcChannels.portfolioRecognizeLedgerImportScreenshots, (event, input: { sourcePaths: string[] }) => {
    assertTrustedSender(event, window);
