...
交易日记围绕「过程可追踪、风险可度量、复盘可改进」设计，把分散在表格、备忘录和提醒 App 里的碎片工作，收拢到一台本地桌面工作台。
...
| 定投纪律如何持续执行？   | 基金定投计划、期次日历、扣款确认与逾期跟踪     |
...
- **基金定投**：管理 OTC / ETF / LOF 定投计划，跟踪期次扣款、逾期与纪律统计。
...
| 设计要点       | 说明                                                          |
| -------------- | ------------------------------------------------------------- |
| 本地优先       | 核心记录默认写入本机 SQLite，无需云端账号即可使用             |
| 敏感凭据隔离   | OpenRouter API Key、启动访问密码等存放在 userData，不进入备份 |
| 可选启动访问锁 | 设置页可开启启动密码，scrypt 哈希本地保存                     |
| 离线 License   | Pro 激活码本机离线验证，不上传交易数据                        |
| 不代客交易     | 不提供买卖建议，不连接券商自动下单                            |
| 开源可审计     | MIT 协议开源，可自行审查存储、备份与导出逻辑                  |

应用内 **侧栏 → 关于我们** 页面有更完整的产品介绍、安全说明与社区入口。

## 社区与支持

交易日记由独立开发者维护。欢迎加入微信交流群反馈问题、交流用法；若产品对你有帮助，也欢迎自愿赞赏支持后续开发。

<table>
  <tr>
    <td width="50%" align="center">
      <img src="src/renderer/public/about/community-wechat-group.png" alt="微信交流群" width="220" />
      <br /><strong>微信交流群</strong><br /><em>交易为生，复盘为师</em>
    </td>
    <td width="50%" align="center">
      <img src="src/renderer/public/about/donate-wechat.png" alt="微信赞赏" width="220" />
      <br /><strong>微信赞赏</strong><br /><em>自愿支持，不影响任何功能</em>
    </td>
  </tr>
</table>

> 群二维码会定期更新。若无法入群，请在 [Issues](https://github.com/minimissile/trading-diary/issues) 留言，或在已安装客户端的「关于我们」页面查看最新二维码。
